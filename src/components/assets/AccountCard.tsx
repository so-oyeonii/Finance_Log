    ...
    <div className="bg-white rounded-xl shadow-sm p-4 hover:shadow-md transition-shadow animate-slide-up">
      <div className="flex items-start justify-between">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-slate-600 truncate">{account.name}</p>
          <p className={cn(
            'text-lg font-bold mt-1',
            isCreditCard ? 'text-red-500' : 'text-slate-800'
          )}>
          ...
          {showReturnRate && account.principal > 0 && (
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs text-slate-400">원금 {formatKRW(account.principal)}</span>
              <span className={cn(
                'text-xs font-medium',
