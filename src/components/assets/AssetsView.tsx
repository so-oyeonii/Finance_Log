      ...
      <div>
        ...
        {bankNames.length > 0 ? (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-8 text-center animate-fade-in">
            ...
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">등록된 계좌가 없습니다</p>
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">입출금 계좌 하나만 추가하면 바로 지출을 기록할 수 있습니다</p>
            <button
              type="button"
              onClick={handleAddAccount}
              className={cn(
                'mt-4 inline-flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-xs font-semibold text-white transition-colors',
                isGraduate ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-emerald-600 hover:bg-emerald-700'
              )}
            >
