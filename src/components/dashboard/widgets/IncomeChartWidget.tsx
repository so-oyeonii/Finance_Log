...
export function IncomeChartWidget({ monthlyStats }: IncomeChartWidgetProps) {
  const chartData = useMemo(
    () => monthlyStats.map((s) => ({ name: s.label, value: s.income })),
    [monthlyStats]
  );
      ...
      <div className="h-44">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: -20 }}>
            ...
            <Tooltip
              formatter={(value: number) => [formatKRW(value), '수입']}
              contentStyle={{ fontSize: 12, borderRadius: 8, border: 'none', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}
            />
            <Bar dataKey="value" fill="#60a5fa" radius={[4, 4, 0, 0]} maxBarSize={28} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
