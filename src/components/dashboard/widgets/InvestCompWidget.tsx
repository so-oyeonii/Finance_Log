    ...
    return (
      <div className="bg-white rounded-xl shadow-sm p-4 animate-fade-in">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-bold text-slate-700">보유 종목 Top 5</h3>
        </div>
        ...
        {top5.map((h, idx) => (
          <div key={`${h.market}_${h.ticker}`} className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-slate-700">{h.ticker}</p>
                <p className="text-[10px] text-slate-400">{h.market}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-xs font-medium text-slate-700">{formatKRW(h.valuation)}</p>
              <p className={cn(
