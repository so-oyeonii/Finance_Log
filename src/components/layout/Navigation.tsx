...
export function Navigation() {
  ...
  const activeColor = mode === 'graduate' ? 'text-indigo-700 dark:text-indigo-400' : 'text-emerald-700 dark:text-emerald-400';
  const activeBorder = mode === 'graduate' ? 'border-indigo-100 dark:border-indigo-800' : 'border-emerald-100 dark:border-emerald-800';
      ...
      {TABS.map((tab) => (
        <button
          ...
          className={`flex-1 py-3 px-3 min-w-[80px] rounded-lg flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2 text-xs md:text-sm font-medium transition-colors ${
            activeTab === tab.id
              ? `bg-white dark:bg-slate-800 ${activeColor} shadow border ${activeBorder}`
