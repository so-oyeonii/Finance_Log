...
export function DividendSection({ dividendStats, mode }: DividendSectionProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 animate-fade-in">
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-500">연간 배당금</span>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
            ...
            <span className="text-xs text-slate-400 dark:text-slate-500">월 평균</span>
          </div>
      ...
      {activeMonths.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 animate-slide-up">
          <div className="flex items-center gap-2 mb-4">
            <BarChart3 className="w-4 h-4 text-amber-500" />
            <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200">월별 배당금</h3>
          </div>
                ...
                <div key={stat.month} className="flex items-center gap-2">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-1">
                      ...
                      />
                      <span className="text-xs text-slate-400 dark:text-slate-500 shrink-0">{formatKRW(stat.totalDividend)}</span>
                    </div>
                  </div>
                </div>
              );
      ...
      {activeMonths.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm p-4 animate-slide-up">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-3">종목별 배당 내역</h3>
          <div className="space-y-3">
            {activeMonths.map((stat) => (
              <div key={stat.month}>
                <p className="text-xs text-slate-400 dark:text-slate-500 font-medium mb-1">{stat.label}</p>
                <div className="space-y-1">
                  {stat.breakdown.map((b) => (
                    <div key={b.ticker} className="flex items-center justify-between text-xs pl-2">
                      <span className="text-slate-600 dark:text-slate-300">{b.ticker}</span>
