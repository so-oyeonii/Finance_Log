...
DROP POLICY IF EXISTS "Users can manage own accounts" ON sl_accounts;
...
DROP POLICY IF EXISTS "Users can manage own transactions" ON sl_transactions;
CREATE POLICY "Users can manage own transactions" ON sl_transactions
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- 3. sl_stocks
CREATE TABLE IF NOT EXISTS sl_stocks (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  local_id integer NOT NULL,
  date text NOT NULL DEFAULT '',
...
DROP POLICY IF EXISTS "Users can manage own stocks" ON sl_stocks;
CREATE POLICY "Users can manage own stocks" ON sl_stocks
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- 4. sl_stock_prices
CREATE TABLE IF NOT EXISTS sl_stock_prices (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  key text NOT NULL,
  price numeric NOT NULL DEFAULT 0,
...
DROP POLICY IF EXISTS "Users can manage own stock_prices" ON sl_stock_prices;
CREATE POLICY "Users can manage own stock_prices" ON sl_stock_prices
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- 5. sl_savings
CREATE TABLE IF NOT EXISTS sl_savings (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  local_id integer NOT NULL,
  type text NOT NULL DEFAULT '',
...
DROP POLICY IF EXISTS "Users can manage own savings" ON sl_savings;
CREATE POLICY "Users can manage own savings" ON sl_savings
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- 6. sl_recurring
CREATE TABLE IF NOT EXISTS sl_recurring (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  local_id integer NOT NULL,
  type text NOT NULL DEFAULT '',
...
DROP POLICY IF EXISTS "Users can manage own recurring" ON sl_recurring;
